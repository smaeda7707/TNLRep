...
{
    public class BrownChestWorldGen : ModSystem
    {
        ...
        public override void PostWorldGen()
        {
            ...
            int[] itemsToPlaceInWaterChests = { ModContent.ItemType<Sparkler>(), ModContent.ItemType<NatureBow>(), ModContent.ItemType<LargeSpring>()};
            ...
            for (int chestIndex = 0; chestIndex < Main.maxChests; chestIndex++)
            {
                ...
                if (chestTile.TileType == TileID.Containers && chestTile.TileFrameX == 0 * 36)
                {
                    ...
                    {
                        if (chest.item[inventoryIndex].type == ItemID.Spear || chest.item[inventoryIndex].type == ItemID.WoodenBoomerang || chest.item[inventoryIndex].type == ItemID.WandofSparking || chest.item[inventoryIndex].type == ItemID.Aglet || chest.item[inventoryIndex].type == ItemID.ClimbingClaws || chest.item[inventoryIndex].type == ItemID.Umbrella || chest.item[inventoryIndex].type == ItemID.Blowpipe)
                        {
                            // Place the item
