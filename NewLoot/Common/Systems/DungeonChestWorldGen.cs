...
namespace NewLoot.Common.Systems.GenPasses
{
    public class DungeonChestWorldGen : ModSystem
    {
        ...
        public override void PostWorldGen()
        {
            ...
            int[] itemsToPlaceInWaterChests = { ModContent.ItemType<BladeShotgun>(), ModContent.ItemType<CrescentSickle>(), ModContent.ItemType<DungeonWhip>(), ModContent.ItemType<RainOfChains>() };
            ...
            for (int chestIndex = 0; chestIndex < Main.maxChests; chestIndex++)
            {
                ...
                if (chestTile.TileType == TileID.Containers && chestTile.TileFrameX == 2 * 36)
                {
                    ...
                    {
                        if (chest.item[inventoryIndex].type == ItemID.Muramasa || chest.item[inventoryIndex].type == ItemID.AquaScepter || chest.item[inventoryIndex].type == ItemID.BubbleGun || chest.item[inventoryIndex].type == ItemID.CobaltShield || chest.item[inventoryIndex].type == ItemID.BlueMoon || chest.item[inventoryIndex].type == ItemID.MagicMissile || chest.item[inventoryIndex].type == ItemID.Valor || chest.item[inventoryIndex].type == ItemID.Handgun)
                        {
                            // Place the item
