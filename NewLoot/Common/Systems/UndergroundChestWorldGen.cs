...
{
    public class UndergroundChestWorldGen : ModSystem
    {
        ...
        public override void PostWorldGen()
        {
            ...
            int[] itemsToPlaceInWaterChests = { ModContent.ItemType<Blunderbuss>(), ModContent.ItemType<SwiftBoots>(), ModContent.ItemType<SharpeningBlock>() };
            ...
            for (int chestIndex = 0; chestIndex < Main.maxChests; chestIndex++)
            {
                ...
                if (chestTile.TileType == TileID.Containers && chestTile.TileFrameX == 1 * 36)
                {
                    ...
                    {
                        if (chest.item[inventoryIndex].type == ItemID.HermesBoots || chest.item[inventoryIndex].type == ItemID.ShoeSpikes || chest.item[inventoryIndex].type == ItemID.FlareGun || chest.item[inventoryIndex].type == ItemID.Mace || chest.item[inventoryIndex].type == ItemID.BandofRegeneration || chest.item[inventoryIndex].type == ItemID.EnchantedBoomerang)
                        {
                            // Place the item
