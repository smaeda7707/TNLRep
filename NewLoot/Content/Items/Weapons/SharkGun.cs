...
{
    internal class SharkGun : ModItem
    {
        public static int gunLoad = 0;
        private int reloadUseSpeed = 30;
        ...
        public override void SetDefaults()
        {
            ...
            Item.knockBack = 7.2f; // Sets the item's knockback. Note that projectiles shot by this weapon will use its and the used ammunition's knockback added together.
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            else
            {
                if (gunLoad == maxLoad)
                {
                    ...
                    // Create a projectile.
                    Projectile.NewProjectileDirect(source, position, newVelocity * 3, type = ModContent.ProjectileType<MiniSharkronGun>(), damage * 4, knockback *= 1.5f, player.whoAmI);
                    gunLoad = 0;
                }
                else
                {
                    Item.shootSpeed = 23f;
                    Projectile.NewProjectileDirect(source, position, velocity, type = ProjectileID.BulletHighVelocity, damage, knockback, player.whoAmI);
                    gunLoad -= 1;
                }
