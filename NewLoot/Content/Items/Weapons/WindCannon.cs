...
{
    internal class WindCannon : ModItem
    {
        public static int gunLoad = 0;
        private int reloadUseSpeed = 38;
        ...
        public override void SetDefaults()
        {
            ...
            Item.knockBack = 10.5f; // Sets the item's knockback. Note that projectiles shot by this weapon will use its and the used ammunition's knockback added together.
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            else
            {
                if (gunLoad == maxLoad)
                {
                    ...
                    // Create a projectile.
                    Projectile.NewProjectileDirect(source, position, velocity * 5, type = ModContent.ProjectileType<Whirlwind>(), damage/10, knockback * 1.5f, player.whoAmI);
                    SoundEngine.PlaySound(SoundID.Item113, player.Center);
                    gunLoad -= 2;
                }
                else
                {
                    Projectile.NewProjectileDirect(source, position, velocity, type = ModContent.ProjectileType<WindBomb>(), damage, knockback, player.whoAmI);
                    gunLoad -= 1;
                }
        ...
        public override bool CanUseItem(Player player)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    Item.shootSpeed = 0.5f; // The speed of the projectile (measured in pixels per frame.)
