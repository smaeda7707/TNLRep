...
namespace NewLoot.Content.Projectiles
{
    ...
    public class CrescentSickleProjSecondary : ModProjectile
    {
        public int count;
        ...
        private const float SWINGRANGE = 1f * (float)Math.PI; // The angle a swing attack covers (300 deg) (DEFAULT 1f)
        ...
        private const float SPINRANGE = 17.5f * (float)Math.PI; // The angle a spin attack covers (630 degrees)
        ...
        private const float SPINTIME = 7.75f; // How much longer a spin is than a swing (DEFAULT 3.1f)
        ...
        private float prepTime => 2f / Owner.GetTotalAttackSpeed(Projectile.DamageType);
        ...
        public override string Texture => "NewLoot/Content/Items/Weapons/CrescentSickle"; // Use texture of item as projectile texture
        ...
        {
            Projectile.width = 60; // Hitbox width of projectile
            Projectile.height = 58; // Hitbox height of projectile
            Projectile.friendly = true; // Projectile hits enemies
            Projectile.timeLeft = 10000; // Time it takes for projectile to expire
            Projectile.penetrate = -1; // Projectile pierces infinitely
            Projectile.tileCollide = false; // Projectile does not collide with tiles
            Projectile.usesLocalNPCImmunity = true; // Uses local immunity frames
            Projectile.localNPCHitCooldown = 10; // We set this to -1 to make sure the projectile doesn't hit twice
        ...
        public override void OnSpawn(IEntitySource source)
        {
            ...
            Projectile.NewProjectile(source, Projectile.position, Projectile.velocity * 0, ModContent.ProjectileType<Cyclone>(), Projectile.damage / 5, 15.75f);
        ...
        {
            Vector2 velocity = Projectile.velocity.RotatedByRandom(MathHelper.ToRadians(32));
            velocity *= Main.rand.NextFloat(0.8f, 1.3f);
            if (count >= 3)
            {
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, velocity * 12, ModContent.ProjectileType<CrescentBeam>(), Projectile.damage / 2, Projectile.knockBack / 2, Main.myPlayer);
                count = 0;
            }
        ...
        public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
        {
            ...
            // If the NPC is hit by the spin attack, decrease knockback slightly
            if (CurrentAttack == AttackType.Spin)
        ...
        private void ExecuteStrike()
        {
                ...
                {
                    SoundEngine.PlaySound(SoundID.DD2_MonkStaffSwing); // Play sword sound again
