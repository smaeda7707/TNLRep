...
namespace NewLoot.Content.Projectiles
{
    ...
    public class CrescentSickleProjectile : ModProjectile
    {
        ...
        private const float SWINGRANGE = 1.4f * (float)Math.PI; // The angle a swing attack covers (300 deg) (DEFAULT 1f)
        ...
        private const float SPINTIME = 3.1f; // How much longer a spin is than a swing
        ...
        private void UnwindStrike()
        {
            if (CurrentAttack == AttackType.Swing)
            {
                if (count >= 3)
                {
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 10, ModContent.ProjectileType<CrescentBeam>(), Projectile.damage / 2, Projectile.knockBack / 2, Main.myPlayer);
                    count = 0;
                }
