...
{
    internal class DemonYoyoProj : ModProjectile
    {
        private int count = 0;
        public override void SetStaticDefaults()
        {
            ...
            ProjectileID.Sets.YoyosLifeTimeMultiplier[Projectile.type] = 12.25f;
            ...
            ProjectileID.Sets.YoyosTopSpeed[Projectile.type] = 12.2f;
        }
        ...
        {
            count ++;
            if (Main.player[Projectile.owner].HasBuff(ModContent.BuffType<DemonYoyoBuff>()) && count == 10)
            {
                Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 0, ModContent.ProjectileType<DemonYoyoEnergy>(), Projectile.damage / 2, 0f, 0, 1);
                count = 0;
            }
            else if (Main.player[Projectile.owner].HasBuff(ModContent.BuffType<DemonYoyoBuff>()))
            {
                Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 0, ModContent.ProjectileType<DemonYoyoEnergy>(), Projectile.damage * 0, 0f, 0, 1);
            }
    ...
    internal class DemonYoyoEnergy : ModProjectile
    {
        Color color = new Color(88, 81, 199);
        public override void SetDefaults()
        {
            Projectile.width = 16;
