...
namespace NewLoot.Content.Projectiles
{
    ...
    internal class SlimeMaceProjectile : ModProjectile
    {
        private const string ChainTexturePath = "NewLoot/Content/Projectiles/SlimeMaceProjectileChain"; // The folder path to the flail chain sprite
        private const string ChainTextureExtraPath = "NewLoot/Content/Projectiles/SlimeMaceProjectileChain2";  // This texture and related code is optional and used for a unique effect
        ...
        public override void AI()
        {
            ...
            float maxRetractSpeed = 9f; // The max speed the projectile will have while retracting
            ...
            if (timer == 45) {
                ...
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, launchVelocity, ModContent.ProjectileType<SlimeBall>(), Projectile.damage, Projectile.knockBack / 2, Main.myPlayer, 0, 1);

                timer = 0;
            }
            if (player.GetModPlayer<SlimeMaceBuffPlayer>().active)
            {
                if (Main.myPlayer == Projectile.owner && Projectile.ai[0] == 2f && Projectile.ai[1] == 0f)
                {
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 2, ModContent.ProjectileType<SlimeBallBig>(), Projectile.damage * 2, Projectile.knockBack, Main.myPlayer);
                    Projectile.ai[1]++;
                }
            }
			...
			*/
            ...
            if (Main.rand.NextBool(dustRate))
                Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.t_Slime, 0f, 0f, 150, default(Color), 1.3f);
        }
