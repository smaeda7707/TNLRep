...
{
    internal class TinWhipProj : ModProjectile
    {
        ...
        public override void SetDefaults()
        {
            ...
            Projectile.WhipSettings.RangeMultiplier = 0.54f;
        ...
        public override void AI()
        {
            ...
            if (Utils.GetLerpValue(0.1f, 0.7f, swingProgress, clamped: true) * Utils.GetLerpValue(0.9f, 0.7f, swingProgress, clamped: true) > 0.5f && !Main.rand.NextBool(3))
            {
                ...
                int dustType = DustID.Tin;
                if (Main.rand.NextBool(3))
                    dustType = DustID.Tin;

                // After choosing a randomized dust and a whip segment to spawn from, dust is spawned.
                Dust dust = Dust.NewDustDirect(spawnArea.TopLeft(), spawnArea.Width, spawnArea.Height, dustType, 0f, 0f, 100, Color.Wheat);
            ...
            if (Main.player[Projectile.owner].GetModPlayer<NewLootPlayer>().tinWhipbuffTimer > 0)
            {
                Projectile.ArmorPenetration = 3;
            }

            Main.player[Projectile.owner].GetModPlayer<NewLootPlayer>().tinWhipbuffTimer--;
        }


        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            Projectile.damage = (int)(Projectile.damage * 0.78f); // Multihit penalty. Decrease the damage the more enemies the whip hits.
            if (Main.player[Projectile.owner].HasBuff(ModContent.BuffType<TinWhipBuff>()))
            {
                Main.player[Projectile.owner].GetModPlayer<NewLootPlayer>().tinWhipbuffTimer = 70;
            }
        }

        private void DrawLine(List<Vector2> list)
        {
            Color colorUse = new Color(58, 36, 37);
            ...
            for (int i = 0; i < list.Count - 1; i++)
            {
                ...
                Color color = Lighting.GetColor(element.ToTileCoordinates(), colorUse);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int i = 0; i < list.Count - 1; i++)
            {
                ...
                if (i == list.Count - 2)
                {
                    ...
                    frame.Y = 84; // Distance from the top of the sprite to the start of the frame.
                    frame.Height = 14; // Height of the frame.
                    frame.Width = 14;
